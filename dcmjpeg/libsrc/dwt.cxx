...
/* </summary>                            */
 ...
 void dwt_encode_stepsize(int stepsize, int numbps, opj_stepsize_t *bandno_stepsize) {
	int p, n;
...
static void v4dwt_interleave_h(v4dwt_t*  w, float*  a, int x, int size){
	float*  bi = (float*) (w->wavelet + w->cas);
	int count = w->sn;
	int i, k;
	for(k = 0; k < 2; ++k){
		for(i = 0; i < count; ++i){
